...
int main(int argc, char **argv) {
  ...
  */
  for (int tst = 1; tst < argc; tst ++)
  {
    ...
    /*
    dut->eval(); mycpu->eval();
    dut->clk = 0; mycpu->clk = 0;
    dut->eval(); mycpu->eval();
    */
    // printf("initial check\n");
    // if (cycle_check(dut,mycpu) == false) return 0;
    // printf("initial check PASS\n");
    bool pass = false;
    for (int i = 0; i < 10000; i++) {
      ...
      if(mycpu->wb == 1){
        next_cycle_r(dut);
        // printf("reg1: %x\n", dut->dbgregs_1);
        // if (tst == 2) print_reg(dut, mycpu);
        if(cycle_check(dut,mycpu) == false){
          print_reg(dut,mycpu);
          return 0;
        }
      }
      // if (mycpu->dbg_pc != 0x80000570) printf("%d %d\n", mycpu->done, dut->done);
      if(dut->done == 1 && mycpu->done == 1){
        printf("Pass Test\n"); pass = true;
        // cycle_check(dut, mycpu);
        // printf("%x %x\n", dut->dbgregs_1, mycpu->dbgregs_1);
        // printf("%x %x\n", dut->dbgregs_2, mycpu->dbgregs_2);
        // print_reg(dut, mycpu);
        break;
